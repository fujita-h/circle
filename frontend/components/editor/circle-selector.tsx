...
import { ReadNotePermissionBadge, WriteNoteConditionBadge } from '@/components/circles/badges';
import { CheckIcon, ChevronUpDownIcon } from '@heroicons/react/20/solid';

export function CircleSelector({ circleId, onChange }: { circleId?: string; onChange?: Function }) {
  const environment = useEnvironment();
  const { instance, accounts } = useMsal();
  const account = useAccount(accounts[0] || {});
  const [selected, setSelected] = useState<any>({});
  const fetcher = swrMsalTokenFetcher(instance, account, environment);
  const { data, isLoading } = useSWR(`${environment.BACKEND_ENDPOINT}/user/circles/postable`, fetcher, {
    revalidateOnFocus: false,
  });

  useEffect(() => {
    if (!data) return;
    const sel = data.find((circle: any) => circle.id === circleId);
    if (sel) {
      setSelected(sel);
    } else {
      setSelected({});
                    ...
                    {({ selected, active }) => (
                      <>
                        <span className={classNames(selected ? 'font-semibold' : 'font-normal', 'block truncate space-x-2')}>
                          <span>{circle.name || circle.id}</span>
