...
import { UpdatePhotoForm, UpdateCircleForm } from '@/components/circles/settings';
...
export default function Page({ params }: { params: any }) {
  ...
  const { data, error, isLoading } = useSWR<any>(`${environment.BACKEND_ENDPOINT}/circles/handle/${handle}`, fetcher, {
    ...
    <>
      ...
      <div className="divide-y divide-white/5">
        <div className="grid max-w-7xl grid-cols-1 gap-x-8 gap-y-10 px-4 py-16 sm:px-6 md:grid-cols-3 lg:px-8">
          ...
          <Suspense fallback={<div>Loading...</div>}>
            <UpdatePhotoForm circleId={data.id} />
          </Suspense>
        </div>
      </div>
      ...
      <div className="divide-y divide-white/5">
        <div className="grid max-w-7xl grid-cols-1 gap-x-8 gap-y-10 px-4 py-16 sm:px-6 md:grid-cols-3 lg:px-8">
          ...
          <UpdateCircleForm circleId={data.id} />
        </div>
      </div>
    </>
  );
